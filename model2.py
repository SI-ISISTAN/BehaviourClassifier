...
def NeuralNet(input_shape):
    ...
    x = Conv1D(filters=32, kernel_size=2, padding='valid')(x)
...
# X_train, X_test, y_train, y_test = train_test_split(mensajes, reacciones, test_size=0.0)
...
epochs = 100
history = modelo_basico.fit(x=mensajes, y=reacciones,
                            validation_split=0.1, epochs=epochs, batch_size=32, verbose=2, shuffle=True)
